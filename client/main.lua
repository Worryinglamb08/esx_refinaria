...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('refinador_clothes_civil'), value = 'citizen_wear'},
				{label = _U('refinador_clothes_refinador'), value = 'refinaria_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'refinaria_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'refinaria_actions_menu'
...
function OpenVigneActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'refinaria_actions',
		{
			title    = 'Refinaria - Petrobras',
			align    = 'top-left',
			elements = elements
		},
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'refinaria', function(data, menu)
					menu.close()
				end)
			end
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'refinaria', vehicleProps)
		...
		local elements = {
			{label = 'Cavalinho Hauler',  value = 'hauler'},
			{label = 'Tanque de Cobustivel',  value = 'tanker'},
		}
...
function OpenMobileVigneActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_refinaria_actions',
		{
			title    = 'refinaria',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_refinaria', 'Refinaria', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_refinadorjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_refinadorjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_refinadorjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_refinadorjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_refinadorjob:hasEnteredMarker', function(zone)
	if zone == 'Mina' and PlayerData.job ~= nil and PlayerData.job.name == 'refinaria'  then
		CurrentAction     = 'petroleo_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TratamentoEtanol' and PlayerData.job ~= nil and PlayerData.job.name == 'refinaria'  then
		CurrentAction     = 'etanol_traitement'
		CurrentActionMsg  = _U('press_traitementf')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TratamentoDiesel' and PlayerData.job ~= nil and PlayerData.job.name == 'refinaria'  then
		CurrentAction     = 'diesel_traitement'
		CurrentActionMsg  = _U('press_traitementa')
		CurrentActionData = {zone = zone}
	end
	
	if zone == 'TratamentoGasolina' and PlayerData.job ~= nil and PlayerData.job.name == 'refinaria'  then
		CurrentAction     = 'gasolina_traitement'
		CurrentActionMsg  = _U('press_traitementc')
		CurrentActionData = {zone= zone}
	end		
			
	if zone == 'Venda' and PlayerData.job ~= nil and PlayerData.job.name == 'refinaria'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'RefinariaAcoes' and PlayerData.job ~= nil and PlayerData.job.name == 'refinaria' then
		CurrentAction     = 'refinaria_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'refinaria' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'refinaria' then
...
AddEventHandler('esx_refinadorjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'Mina') and PlayerData.job ~= nil and PlayerData.job.name == 'refinaria' then
		TriggerServerEvent('esx_refinadorjob:stopHarvest')
	end  
	if (zone == 'TratamentoEtanol' or zone == 'TratamentoDiesel' or zone == 'TratamentoGasolina' ) and PlayerData.job ~= nil and PlayerData.job.name == 'refinaria' then
		TriggerServerEvent('esx_refinadorjob:stopTransform')
	end
	if (zone == 'Venda') and PlayerData.job ~= nil and PlayerData.job.name == 'refinaria' then
		TriggerServerEvent('esx_refinadorjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.RefinariaAcoes.Pos.x, Config.Zones.RefinariaAcoes.Pos.y, Config.Zones.RefinariaAcoes.Pos.z)
		...
		AddTextComponentString("Refinaria - Petrobras")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'refinaria' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'refinaria' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_refinadorjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_refinadorjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'refinaria' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'petroleo_harvest' then
					TriggerServerEvent('esx_refinadorjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'etanol_traitement' then
					TriggerServerEvent('esx_refinadorjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'diesel_traitement' then
					TriggerServerEvent('esx_refinadorjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'gasolina_traitement' then
					TriggerServerEvent('esx_refinadorjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_refinadorjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'refinaria_actions_menu' then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'refinaria', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'refinadora' and (GetGameTimer() - GUI.Time) > 150 then
